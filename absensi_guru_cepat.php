...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_quick_absensi_guru'])) {
    ...
    if (empty($guru_id)) {
        ...
        foreach ($siswaOptions as $siswa) {
            ...
            $checkQuery = "SELECT id_absensi_guru FROM absensi_guru WHERE tgl = ? AND siswa_id = ? AND guru_id = ? AND kelas_id = ?";
            $checkStmt = mysqli_prepare($conn, $checkQuery);
            mysqli_stmt_bind_param($checkStmt, 'siii', $tgl, $siswa_id, $guru_id, $kelas_id);
            mysqli_stmt_execute($checkStmt);
            $checkResult = mysqli_stmt_get_result($checkStmt);
            
            if (mysqli_num_rows($checkResult) > 0) {
                // Update existing record
                $updateQuery = "UPDATE absensi_guru SET status = ? WHERE tgl = ? AND siswa_id = ? AND guru_id = ? AND kelas_id = ?";
                $updateStmt = mysqli_prepare($conn, $updateQuery);
                mysqli_stmt_bind_param($updateStmt, 'ssiii', $status, $tgl, $siswa_id, $guru_id, $kelas_id);
                ...
                // Insert new record
                $insertQuery = "INSERT INTO absensi_guru (tgl, guru_id, kelas_id, siswa_id, status) VALUES (?, ?, ?, ?, ?)";
                $insertStmt = mysqli_prepare($conn, $insertQuery);
                mysqli_stmt_bind_param($insertStmt, 'siiis', $tgl, $guru_id, $kelas_id, $siswa_id, $status);
...
<div class="content">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="fw-semibold mb-1">Absensi Cepat Guru</h4>
    ...
    <?php if (!empty($siswaOptions)): ?>
        <div class="card p-4">
            ...
            <form method="POST">
                ...
                <input type="hidden" name="guru_id" value="<?= htmlspecialchars($selectedGuru); ?>">
                ...
                <div class="d-flex gap-2">
                    <button type="submit" name="submit_quick_absensi_guru" class="btn btn-success">
