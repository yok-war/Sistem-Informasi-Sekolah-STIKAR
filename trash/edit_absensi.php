...
<div class="content container mt-4">
    <div class="card shadow-sm mt-4">
        <div class="card-header">
            <h5 class="mb-0">Ubah Data Absensi</h5>
        </div>

        <div class="card-body">
            <form method="post">
                <div class="row g-3">
                    <input type="hidden" name="id" value="<?= $absensi['id_absensi'] ?>">

                    <div class="col-md-6">
                        <label class="form-label">Siswa</label>
                        <select name="siswa" class="form-select" required>
                            <?php foreach ($siswa as $row) : ?>
                                <option value="<?= $row['id_siswa'] ?>" <?= $row['id_siswa'] == $absensi['siswa_id'] ? 'selected' : '' ?>>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="status" class="form-select" required>
                            <?php foreach ($statusOptions as $value => $label) : ?>
                                <option value="<?= $value ?>" <?= $absensi['status'] === $value ? 'selected' : '' ?>>
                                    <?= $label ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="guru" class="form-select">
                            ...
                            <?php foreach ($guru as $row) : ?>
                                <option value="<?= $row['id_guru'] ?>" <?= $row['id_guru'] == $absensi['guru_id'] ? 'selected' : '' ?>>
                                    <?= $row['nama_guru'] ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="col-md-3">
                        ...
                        <input type="text" name="semester" class="form-control" value="<?= $absensi['semester'] ?>">
                    </div>

                    <div class="col-md-3">
                        <label class="form-label">Tahun Ajaran</label>
                        <input type="text" name="tahun_ajaran" class="form-control" value="<?= $absensi['tahun_ajaran'] ?>">
                    </div>

                    <div class="col-12">
                        <div class="form-check mt-2">
                            <input class="form-check-input" type="checkbox" name="is_verified" id="is_verified" <?= (int) $absensi['is_verified'] === 1 ? 'checked' : '' ?>>
